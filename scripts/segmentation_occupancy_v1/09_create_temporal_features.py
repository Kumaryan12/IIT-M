...
IMPORTANCE_CSV = OUTPUT_DIR / "temporal_no_env_feature_importance.csv"
...
def is_env_related(col):
    return any(k in col for k in ENV_RELATED_KEYWORDS)
...
def main():
    ...
    if env_related_cols:
        print("\nRemoving environment-related columns from no-env analysis:")
        print(env_related_cols)

        X = X.drop(columns=env_related_cols)

    X, constant_cols = drop_constant_columns(X)
    X, corr_cols = drop_high_corr_numeric_columns(X)
    ...
    print("\nTop 40 temporal TRUE no-env features:")
